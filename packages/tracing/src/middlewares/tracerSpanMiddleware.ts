...
export function tracerSpanMiddleware(): MiddlewareHandler {
	return async (c, next) => {
		...
		// Try to get userId from request if available (for regular API routes)
		const rawRequest = c.req.raw.clone() as { userId?: string };
		if (rawRequest.userId) {
			attributes.userId = rawRequest.userId;
		}
